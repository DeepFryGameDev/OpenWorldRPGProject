...
namespace DeepFry
{
    ...
    public class EquipShopSlotInteraction : MonoBehaviour
    {
        bool visible;
        ...
        GameObject equipFieldPrefab;
        GameObject prefabView;

        GameObject quantityCanvas;
        ...
        void Start()
        {
            ...
            equipFieldPrefab = equip.fieldPrefab;
            prefabView = GameObject.Find("PrefabView/ItemPrefab/PrefabView");
    ...
    "EquipRowPanel/EquipRow/RightHandEquipSlot");
        ...
        {
            visible = true;
            if (!si.inQuantitySelect) ShowEquipDetails();
        }

        public void Exit()
        {
            visible = false;
            if (!si.inQuantitySelect) HideEquipDetails();
        }
        ...
        void SetPrefabView()
        {
            GameObject prefabToRender = Instantiate(equip.fieldPrefab);
            prefabToRender.layer = si.prefabLayer;

            //disable gravity for all objects
            if (prefabToRender.GetComponent<Rigidbody>() != null)
            {
                prefabToRender.GetComponent<Rigidbody>().useGravity = false;
            }

            foreach (Transform child in prefabToRender.transform)
            {
            ...
            prefabToRender.transform.SetParent(prefabView.transform, false);
        }
        ...
        void HideEquipDetails()
        {
            ...
            foreach (Transform child in descObj.transform)
            {
                Destroy(child.gameObject);
            }

            foreach (Transform child in prefabView.transform)
            {
                Destroy(child.gameObject);
            }
        }
